...
mytextclock = awful.widget.textclock(" %a %d %b %Y %T", 0)
...
globalkeys = awful.util.table.join(
   ...
   -- bind PrintScrn to capture a screen
      awful.key({ }, "Print", function () awful.util.spawn_with_shell("import -window root -quality 98 $HOME/Pictures/$(date +%F_%H-%M-%S).png") end), 
   -- ALSA volume control
    awful.key({}, "XF86AudioRaiseVolume",
        function ()
            os.execute(string.format("amixer set %s 3%%+", volumewidget.channel))
            volumewidget.update()
        end),
